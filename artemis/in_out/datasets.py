...
def custom_grounding_dataset_similar_to_affective_loader(grounding_data_csv, loader, img2emo_clf=None, n_workers=None, device=None):
    """
    ...
    if 'grounding_emotion' in df.columns:
        ...
        temp_dataset = ImageClassificationDataset(image_files=image_files,
                                                img_transform=img_trans)
        img_dataloader = DataLoader(temp_dataset, batch_size, num_workers=n_workers)
        emo_pred_distribution = image_to_emotion(img2emo_clf, img_dataloader, device)

        emotions = pd.Series(emo_pred_distribution.argmax(-1).tolist())  # use maximizer of emotions.
    else:
        emotions = dummy
